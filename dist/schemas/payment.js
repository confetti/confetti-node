...
export const PaymentSchema = z.object({
    ...
    amount: z.number().describe(JSON.stringify({
        label: 'Amount',
    })),
    vat: z.number().describe(JSON.stringify({
        label: 'VAT',
    })),
    vatPercentage: z.number().describe(JSON.stringify({
        label: 'Vat %',
    })),
...
export const staticPaymentsResourceOptionsSchema = staticBaseFindAllResourceOptionsSchema.extend({
    filter: z
        ...
        eventId: z.union([z.string(), z.number()]),
        status: z
            .array(z.enum(['paid', 'refunded', 'pending-invoice', 'sent-invoice', 'paid-invoice', 'cancelled-invoice']).describe(JSON.stringify({
